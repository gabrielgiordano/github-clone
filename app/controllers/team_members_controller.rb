class TeamMembersController < ApplicationController
  ...
  before_action -> { authorize_user_with(Teams::MemberPolicy, current_user.id, team_id, action_name) }

  def create
    creation = Teams::MemberCreator.execute(team_id, team_params["email"])
  ...
  def destroy
    deletion = Teams::MemberDestroyer.execute(team_id, team_params["user_id"])
